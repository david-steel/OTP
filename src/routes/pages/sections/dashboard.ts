...
import { BASE_URL, bc, renderV7, escapeHtml, OG_DEFAULT } from '../_shared.js';
...
export default async function dashboardRoutes(app: FastifyInstance) {
  ...
  // GET /dashboard/kpis-debug?email=<member email> -- READ-ONLY super-admin
  // diagnostic. Replays the EXACT /dashboard/kpis viewer-scoping path for the
  // member with that email and reports which filter (owner-claim vs team-
  // membership) drops each KPI. Added 2026-06-30 to root-cause why a member
  // (Kristen) sees an empty scoreboard while admins see their KPIs. No writes.
  // Delete once the cause is confirmed and fixed.
  app.get<{ Querystring: { email?: string } }>('/dashboard/kpis-debug', async (request, reply) => {
    // 404 (not 403) so the endpoint is invisible to non-super-admins.
    if (!(request as any).isSuperAdmin) return reply.status(404).send({ error: 'not found' });
    const resolved = await resolveOrgForRequest(request);
    if (!resolved) return reply.status(404).send({ error: 'no org' });
    const { org } = resolved;
    const email = String(request.query.email || '').trim().toLowerCase();
    if (!email) return reply.send({ error: 'pass ?email=<member email>' });

    const members = await db.select().from(orgMembers).where(eq(orgMembers.orgId, org.id));
    const member = members.find(m => (m.email || '').trim().toLowerCase() === email) || null;
    if (!member) {
      return reply.send({
        org: { id: org.id, name: org.name, clerkOrgId: (org as any).clerkOrgId },
        email,
        found: false,
        hint: 'No org_members row in this org with that email.',
        emailsInOrg: members.map(m => m.email).filter(Boolean),
      });
    }

    // Mirror the real route's role + scoping exactly (dashboard.ts ~810-895).
    const role = (member.role || 'managee') as string;
    const isAdminLikeKpis = role === 'owner' || role === 'admin' || role === 'implementer';

    const founderTiles = await getFounderTileIds(org.id, (org as any).clerkOrgId);
    const isLegacyFounder = !!((org as any).clerkOrgId && (org as any).clerkOrgId === member.clerkUserId);

    const rawClaims: string[] = [];
    const cids = (member.claimedEntityIds as unknown) as string[] | null;
    if (Array.isArray(cids)) for (const id of cids) if (typeof id === 'string' && id) rawClaims.push(id);
    if (member.claimedEntityId && !rawClaims.includes(member.claimedEntityId)) rawClaims.push(member.claimedEntityId);
    const claimedSet = new Set(isLegacyFounder ? rawClaims : rawClaims.filter(id => !founderTiles.has(id)));

    const teamRows = await db.select({ teamId: teamMemberships.teamId })
      .from(teamMemberships).where(eq(teamMemberships.memberId, member.id));
    const myTeamIds = new Set(teamRows.map(r => r.teamId));

    const { listKpis } = await import('../../../services/kpi.js');
    const allKpis = await listKpis(org.id, { archived: 'exclude' });

    const kpiVerdicts = allKpis.map((k: any) => {
      const ownerMatch = !!(k.ownerExternalId && claimedSet.has(k.ownerExternalId));
      const teamMatch = !!(k.teamId && myTeamIds.has(k.teamId));
      const visible = isAdminLikeKpis || ownerMatch || teamMatch;
      return {
        title: k.title,
        ...
        teamMatch,
        visible,
        droppedBecause: visible ? null : 'ownerExternalId not in claimedSet AND teamId not in myTeamIds',
      };
    });
    ...
    return reply.send({
      ...
      member: {
        ...
        claimedEntityIds: cids,
      },
      scoping: {
        isAdminLikeKpis,
        isLegacyFounder,
        rawClaims,
        founderTiles: [...founderTiles],
        claimedSetAfterScrub: [...claimedSet],
        // If a KPI's ownerExternalId shows up here, the founder-tile scrub ate
        // the member's own claimed seat -> that is the bug.
        scrubbedOutByFounderFilter: rawClaims.filter(id => !claimedSet.has(id)),
        myTeamIds: [...myTeamIds],
      },
      totalOrgKpis: allKpis.length,
      visibleToThisMember: kpiVerdicts.filter(v => v.visible).length,
      kpis: kpiVerdicts,
    });
  });
