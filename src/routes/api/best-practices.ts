...
const generatorRateLimit = createRateLimiter({ windowMs: 3600000, maxRequests: 5 });
...
export default async function bestPracticesRoutes(app: FastifyInstance) {
  ...
  // ============================================================
  // PUBLIC: GET /api/v1/public/practices/:industry
  // No auth required. Returns original industry practice packs.
  // ============================================================
  app.get<{
    Params: { industry: string };
    Querystring: { category?: string; format?: string };
  }>('/public/practices/:industry', async (request, reply) => {
    const { industry } = request.params;
    const { category, format } = request.query;
    ...
    if (rows.length === 0) {
      return reply.status(404).send({
        error: { code: 'INDUSTRY_NOT_FOUND', message: `No practices found for industry: ${industry}` },
      });
    }
    ...
    const votesResult = await db.execute(sql`
      SELECT pv.best_practice_id, SUM(pv.vote) AS score, COUNT(*) AS vote_count
    ...
    const voteMap: Record<string, { score: number; count: number }> = {};
    for (const row of (votesResult.rows || [])) {
      voteMap[row.best_practice_id] = { score: Number(row.score), count: Number(row.vote_count) };
    }
    ...
    const practices = rows.map(r => ({
      ...
      votes: voteMap[r.id] || { score: 0, count: 0 },
    }));

    // If format=claudemd, return a downloadable CLAUDE.md snippet
    if (format === 'claudemd') {
      ...
      let md = `# Coordination Rules - ${industry.charAt(0).toUpperCase() + industry.slice(1)} Industry\n`;
      md += `# Generated by OTP (orgtp.com) - ${new Date().toISOString().split('T')[0]}\n\n`;
      ...
      for (const [cat, items] of Object.entries(grouped)) {
        ...
        for (const item of items) {
          ...
          md += `${item.definition}\n`;
          if (item.failureMode) {
            md += `**Failure mode:** ${item.failureMode}\n`;
          }
          md += '\n';
        }
      }
      ...
      reply.header('Content-Disposition', `attachment; filename="CLAUDE-${industry}-coordination.md"`);
      return md;
    }
  ...
  // ============================================================
  // PUBLIC: GET /api/v1/public/industries
  // No auth required. Lists available industry practice packs.
  // ============================================================
  app.get('/public/industries', async (_request, _reply) => {
    ...
    return {
      industries: result.filter(r => r.industry).map(r => ({
        slug: r.industry,
  ...
  // ============================================================
  // PUBLIC: POST /api/v1/public/generate-claudemd
  // AI-powered CLAUDE.md generator. Rate limited: 5/IP/hour.
  // Falls back to static template when API unavailable.
  // ============================================================
  app.post('/public/generate-claudemd', async (request, reply) => {
    const ip = request.ip;
    if (!generatorRateLimit(ip)) {
      return reply.status(429).send({
        error: { code: 'RATE_LIMITED', message: 'Generator limit reached (5 per hour). Try again later.' },
      });
    }
    ...
    if (!parsed.success) {
      return reply.status(400).send({
        error: { code: 'INVALID_INPUT', message: 'Invalid input. Provide industry, teamSize (1-500), and agentCount (1-50).' },
      });
    }

    const { industry, teamSize, agentCount, focus } = parsed.data;

    const result = await generateClaudeMd({ industry, teamSize, agentCount, focus });

    // Log to audit
    try {
      await db.insert(auditLogs).values(
        createAuditEntry('claudemd_generated', 'generator', {
          details: { industry, teamSize, agentCount, source: result.source, practiceCount: result.practiceCount },
        })
      );
    } catch (e) {
      // Non-blocking
