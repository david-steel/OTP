...
### R1 — Generalized event bus + org SSE stream — ~3-4 days
- **Changes:** extract `meeting-bus.ts` → `event-bus.ts` keyed by `(orgId, topic)`; meeting rooms become topics (`meeting:<id>`), preserving the existing public surface so meetings.ts/todos.ts/rocks.ts/tickets.ts/headlines.ts call sites keep working. New `GET /api/v1/events/stream` with `getAuthOrg()` auth, topic filters, 25s heartbeat, `Last-Event-ID` replay from `org_events`, `resync` overflow signal, connection cap per org (e.g. 50) + global cap with metric.
- **Flag:** `REALTIME_STREAM_ENABLED`.
- **Test:** Fastify-inject SSE tests (pattern exists in `agents.sse.test.ts`); replay-window tests; **tenancy tests** (org A subscriber must never receive org B events — including impersonation and legacy-founder paths); page/API resolver parity test.
- **Rollback:** flag off → endpoint 404s; meeting bus continues exactly as today.

### R2 — UI consumers, incremental (highest visible value) — ~1-1.5 weeks, shippable per-surface
1. **Meeting page: targeted refetch instead of `location.reload()`.** Per `kind`, refetch only the affected section (issues list, todos list, rocks, scorecard fragment) via existing GETs; keep reload as fallback for unknown kinds. Kills the disruptive mid-meeting reload — directly serves the "OTP must be flawless in live L10s" mandate.
2. **Notification bell + What's New:** subscribe to the org stream from `main.ejs`; drop the 2-min poll to a 10-min fallback. Shared client helper in `public/js/otp-live.js` (versioned `?v=assetVersion` — immutable-cache rule).
3. **Dashboard / scorecard / team pages:** on relevant event → targeted refetch of the data region; subtle "updated just now" affordance. Org chart page: "Chart updated by {name} — refresh" banner first (cheap), live re-render later.
4. **Scorecard auto re-snapshot:** on any KPI value save for a team with an in-progress meeting, server re-snapshots + publishes `{kind:'kpi'}` automatically (generalizes 840b1bd; closes the snapshot-swallows-edits bug class at the root).
- **Rollback:** per-surface; each consumer no-ops if the stream is absent.
