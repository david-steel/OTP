...
**Remaining R2 surfaces (R2.3+, not yet built):**
1. **Targeted per-section swap (dashboard + meeting page):** the "no reload at all" version. Prereq: refactor the page JS to a re-callable init so a swapped-in fragment rebinds, then add server-rendered HTML fragment endpoints per section (issues/todos/rocks/scorecard). The `/l8/meeting/:id` page is the highest-value target (kill the mid-L10 reload) and the most delicate — its own careful, tested increment.
2. **Scorecard auto re-snapshot:** on a KPI value save (incl. agent/Tally pushes) for a team with an in-progress meeting, server re-snapshots + publishes — generalizes the 840b1bd in-meeting-edit fix to ALL value writes. Backend + testable; moderate care (hot KPI path).
