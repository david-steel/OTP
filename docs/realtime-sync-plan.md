...
**Remaining R2 surfaces (R2.2+, not yet built):**
1. **Meeting page: targeted refetch instead of `location.reload()`** — the highest-value but most delicate piece (the large `/l8/meeting/:id` inline JS + draft-snapshot logic). Wants server-rendered HTML *fragment* endpoints per section (issues/todos/rocks/scorecard) so the client swaps a region without duplicating EJS in JS. Deserves its own careful, tested increment; keep `location.reload()` as the fallback for unknown kinds. Deferred deliberately given the "flawless in live L10s" mandate.
2. **Dashboard / scorecard / team pages:** on a relevant event → refetch the data region (needs the same fragment endpoints) or a subtle "Updated — refresh" affordance as a cheap first step.
3. **Scorecard auto re-snapshot:** on a KPI value save (incl. agent/Tally pushes) for a team with an in-progress meeting, server re-snapshots + publishes — generalizes the 840b1bd in-meeting-edit fix to ALL value writes. Backend + testable; moderate care (hot KPI path).
- **Rollback:** per-surface; each consumer no-ops when the stream is absent.
